...
int main(int argc, char *argv[])
{
	...
	{
		pr_out("-----------------------------------");
		pr_out("sbenchmark is server test benchmark");
		pr_out("./sbenchmark <ip> <port> -t 5 ");
		pr_out("-----------------------------------");
		exit(EXIT_SUCCESS);
	} 
...
--------------------------------------");
	...
	pr_out("(result)-----------------");
	//pr_out("real time res: %ld total_t: %ld \n", time_res, total_t);
...
#ifdef JDBG
	//pr_out("[Elapsed Time]: %8ld(sec) %06ld(us). \n total time: %8ld(sec)\n", sec, nsec, time);
...
{
#ifdef JDBG
	perror(msg);	// detailed error log occur
#endif
	pr_err(msg);
	exit(EXIT_FAILURE);
}
